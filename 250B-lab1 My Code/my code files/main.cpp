...
int main (int argc, char** argv) {
	...
	// modify to assign queries to threads
	size_t total = queries.size();
	size_t queriesPerThread = total / thread_cnt;
	size_t extra = total % thread_cnt;
	
	for (size_t i = 0; i < static_cast<size_t>(thread_cnt); i++) {
		size_t start = i * queriesPerThread + std::min((size_t)i, extra);
		size_t count = queriesPerThread + (i < extra ? 1 : 0);
		// traverse_queries now accepts [start, start+count)
		threads[i] = new std::thread(traverse_queries, tree, std::ref(queries), start, start + count, &(return_cnts[i]));
	}
